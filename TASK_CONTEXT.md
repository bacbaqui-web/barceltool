...
- 폴더를 불러오기 전에는 이미지 영역을 숨기고 `assets/youcandoit.png` 안내 이미지, 문구, 불러오기 버튼을 오른쪽 패널의 가로·세로 중앙에 표시한다.
- 중첩 폴더 트리와 폴더 선택
- 폴더 트리 제목 옆의 `하위폴더포함` 토글로 현재 폴더만 보거나 모든 하위 폴더까지 함께 볼 수 있다.
...
- 왼쪽 하단 휴지통 보기와 개수 표시
- 휴지통 이미지 미리보기와 영구 삭제
- 휴지통 보기에서는 오른쪽 상단 작업 버튼이 `복구`, `완전삭제`로 바뀐다.
- 현재 실행 중 휴지통으로 보낸 이미지는 원래 폴더와 이름으로 복구하고, 충돌하면 번호를 붙인다.
- 다시 스캔해 원래 경로 정보가 없는 휴지통 이미지는 최상위 폴더로 복구한다.
- 라이트·다크모드
